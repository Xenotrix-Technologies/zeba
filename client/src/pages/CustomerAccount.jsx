            ...
            <div className="space-y-4">
              {orders.map((ord) => (
                ...
                >
                    ...
                    <div className="flex items-center space-x-3">
                      <span className={`px-3 py-1 rounded-full text-[11px] font-extrabold uppercase ${
                        ord.status === 'delivered'
                          ...
                          : ord.status === 'shipped'
                          ? 'bg-purple-50 text-brand-deepPurple border border-purple-200'
                          : 'bg-amber-50 text-amber-700 border border-amber-200'
                      }`}>
                        {ord.status}
                      </span>
                      <span className="text-xs font-bold text-brand-deepPurple">
                  ...
                  <div className="space-y-2">
                    {ord.items?.map((item, idx) => (
                      <div key={idx} className="flex items-center justify-between text-xs text-brand-darkPurple">
                        ...
                        <span className="font-bold text-brand-deepPurple">₹{parseFloat(item.subtotal_price).toFixed(2)}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  {/* Tracking / Notes */}
                  {ord.notes && (
                    <div className="p-3 rounded-xl bg-brand-softPink/50 border border-brand-primaryPink/20 text-xs text-brand-darkPurple space-y-1">
                      ...
                      <p className="text-[#805A82]">{ord.notes}</p>
                    </div>
                  )}

                  <div className="text-[11px] text-[#805A82] flex items-center justify-between pt-1">
